...
import numpy as np
import learning.mlx.utils as lora_utils
from learning.mlx.utils import tree_flatten
from learning.mlx.models import LoRALinear
...
def load(args):
    ...
    # Get the directory of the notebook (learning folder)
    ...
    if train_file.exists():
        ...
        train = Dataset(str(train_file), args)
    else:
        print(f"Warning: {train_file} not found.")

    if valid_file.exists():
        print(f"Loading validation data from {valid_file}")
        valid = Dataset(str(valid_file), args)
    else:
        print(f"Warning: {valid_file} not found.")

    if test_file.exists():
        print(f"Loading test data from {test_file}")
        test = Dataset(str(test_file), args)
    else:
        print(f"Warning: {test_file} not found.")

    if args.train and train is None:
        raise ValueError(f"Training mode is enabled but {train_file} was not found.")

    if args.train and valid is None:
        print("Warning: Validation file not found. Using a portion of the training data for validation.")
        split = int(0.9 * len(train))
        train, valid = train[:split], train[split:]

    if args.test and test is None:
        raise ValueError(f"Test mode is enabled but {test_file} was not found.")

    print(f"Dataset sizes: Train: {len(train) if train else 0}, "
          f"Validation: {len(valid) if valid else 0}, "
...
def count_parameters(params):
    """Recursively count parameters in nested dictionaries."""
